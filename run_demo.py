...
topo=cmds[2:]
for s in sws:
  setup_veth("s%s-int"%(s,),"s%s-mgr"%(s,))
for s in tors:
  setup_veth("s%s-trf"%(s,),"s%s-tin"%(s,))
for t in topo:
  s1=t[:-1].split("-")[0]
  s2=t[:-1].split("-")[1]
  s1n=s1.split(":")[0][1:]
...
for i in sws:
  ...
  print(switch_args)
  subprocess.Popen(SWITCH_PATH+switch_args,shell=True)
  time.sleep(1)

time.sleep(4)
if args.command_file:
  for i in sws:
    subprocess.Popen(CLI_PATH+"%s %d < s%s-commands.txt"%(args.json_file,args.thrift_port+int(i),i),shell=True).wait()
